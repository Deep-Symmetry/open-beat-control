...
If there is a problem sending the message, an `/error` response will
describe it. Otherwise, if you have a <<playing,`/playing` stream>>
active, any playing state changes will be reported to that stream.
...
If there is a problem sending the message, an `/error` response will
describe it. Otherwise, if you have an <<on-air,`/on-air` stream>>
active, the on-air state changes of the players will be reported on
that stream.
...
If there is a problem sending the message, an `/error` response will
describe it. Otherwise, if you have a <<synced,`/synced` stream>>
active, any sync state changes will be reported to that stream.
